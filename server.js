...
ocR.post('/:id/recibir-bodega', auth, async(req,res)=>{
  ...
  try{
    ...
    const lineas=await client.query('SELECT * FROM ordenes_compra_detalle WHERE oc_id=$1 AND ingresa_bodega=true AND producto_id IS NOT NULL',[req.params.id]);
    if(!lineas.rows.length) throw new Error('No hay lineas marcadas para ingresar a bodega');
